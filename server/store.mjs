...
export async function maintenanceOverview() {
  ...
  return {
    ...
    storage: [
      { key: 'safeCleanup', label: '安全维护可清理项', path: paths.root, bytes: safePreview.estimatedBytes || 0, count: safePreview.matchedCount || 0, protected: false, cleanupLevel: 'safe', cleanupLevelLabel: '可以直接删，不影响工作台', maintenanceType: 'safe-clean', fileActionLabel: '打开项目根目录', actionLabel: '定位安全清理', note: '删的是系统垃圾：.DS_Store、过期临时 JSON、已经没有执行记录对应的空工作区；不会影响任务、模板、AI档案和统计。' },
      ...
      { key: 'workspace', label: '执行工作区 workspace', path: paths.workspaceDir, bytes: workspaceSize, protected: true, cleanupLevel: 'caution', cleanupLevelLabel: '慎重删，会少历史材料', maintenanceType: 'runs', route: '/ai-archive', fileActionLabel: '打开 workspace', actionLabel: '查看 AI档案', note: '这里是执行过程材料和本机工作区；要删必须按 AI档案 / 执行记录预览范围删，避免误删还要查的过程证据。' },
      { key: 'artifacts', label: '执行产物归档 workspace/artifacts', path: paths.artifactDir, bytes: artifactSize, protected: true, cleanupLevel: 'caution', cleanupLevelLabel: '慎重删，会少产物证据', maintenanceType: 'runs', route: '/ai-archive', fileActionLabel: '打开产物归档', actionLabel: '查看 AI档案', note: '这里是美术执行台和 AI档案会查看的归档产物；删了不影响任务本身，但以后可能看不到执行结果证据。' },
      { key: 'artBriefOutputs', label: '美术摘要 outputs/art-briefs', path: path.join(paths.root, 'outputs', 'art-briefs'), bytes: artBriefOutputsSize, protected: false, cleanupLevel: 'range', cleanupLevelLabel: '能删，但要先选范围', maintenanceType: 'art-briefs', fileActionLabel: '打开摘要目录', actionLabel: '定位摘要清理', note: '这里是任务中心生成的摘要 HTML 和 AI 工作说明；删了不影响禅道任务，但对应摘要文件以后就看不到。' },
      { key: 'outputs', label: '输出目录 outputs', path: path.join(paths.root, 'outputs'), bytes: outputsSize, protected: true, cleanupLevel: 'caution', cleanupLevelLabel: '慎重删，别删整个目录', maintenanceType: 'art-briefs', fileActionLabel: '打开 outputs', actionLabel: '定位摘要清理', note: '这里可能混有正式输出和业务产物；不要手动删整个 outputs，只能在维护中心按明确业务类型清理。' },
      { key: 'logs', label: '运行日志 logs', path: path.join(paths.root, 'logs'), bytes: logsSize, protected: false, cleanupLevel: 'range', cleanupLevelLabel: '能删，但排障后再删', route: '/operation-logs', fileActionLabel: '打开 logs', actionLabel: '查看操作日志', note: '这里主要用于排查问题；确认近期不用追查错误后，可以按日期归档或截断，操作日志仍要走范围删除。' }
    ],
    records: [
      { key: 'operationLogs', label: '操作日志', count: Array.isArray(operationLogs) ? operationLogs.length : 0, file: paths.operationLogs, cleanupLevel: 'range', cleanupLevelLabel: '能删旧日志，不影响任务', maintenanceType: 'operation-logs', route: '/operation-logs', fileActionLabel: '打开日志文件目录', actionLabel: '查看日志', note: '删了只是少一段操作审计记录；不会影响执行模板、任务量级、新建执行任务，也不会扣减 AI 产物调用次数。' },
      { key: 'runs', label: '执行记录 / AI档案', count: Array.isArray(runs) ? runs.length : 0, file: paths.runs, cleanupLevel: 'caution', cleanupLevelLabel: '慎重删，会少执行历史', maintenanceType: 'runs', route: '/ai-archive', fileActionLabel: '打开 runs 文件目录', actionLabel: '查看 AI档案', note: '删了会移除非运行中的执行档案、工作区和产物归档；不会删任务中心任务，但以后可能查不到这次执行过程。' },
      ...
      { key: 'artBriefs', label: '美术摘要索引', count: Array.isArray(artBriefs) ? artBriefs.length : 0, file: paths.artBriefs, cleanupLevel: 'range', cleanupLevelLabel: '能删，但要连文件一起删', maintenanceType: 'art-briefs', fileActionLabel: '打开索引文件目录', actionLabel: '定位摘要清理', note: '这是摘要列表索引；必须和 outputs/art-briefs 命中的摘要目录同步删，不能只手动删索引文件。' },
