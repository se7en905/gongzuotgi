...
export async function maintenanceOverview() {
  ...
  return {
    ...
    storage: [
      { key: 'safeCleanup', label: '安全维护可清理项', path: paths.root, bytes: safePreview.estimatedBytes || 0, count: safePreview.matchedCount || 0, protected: false, cleanupLevel: 'safe', cleanupLevelLabel: '可放心清理', maintenanceType: 'safe-clean', actionLabel: '定位安全清理', note: '只包含 .DS_Store、过期临时 JSON 和孤儿工作区，和工作台业务功能无关。' },
      { key: 'data', label: '平台业务数据 data', path: paths.dataDir, bytes: dataSize, protected: true, cleanupLevel: 'protected', cleanupLevelLabel: '禁止手动删目录', actionLabel: '仅查看体量', note: '账号、任务、缓存、调用次数和看板状态，不允许手动删目录。' },
      { key: 'workspace', label: '执行工作区 workspace', path: paths.workspaceDir, bytes: workspaceSize, protected: true, cleanupLevel: 'caution', cleanupLevelLabel: '需慎重', maintenanceType: 'runs', route: '/ai-archive', actionLabel: '查看 AI档案', note: '执行日志、材料和本机执行工作区，必须按执行记录入口范围清理。' },
      { key: 'artifacts', label: '执行产物归档 workspace/artifacts', path: paths.artifactDir, bytes: artifactSize, protected: true, cleanupLevel: 'caution', cleanupLevelLabel: '需慎重', maintenanceType: 'runs', route: '/ai-archive', actionLabel: '查看 AI档案', note: '美术执行台和 AI档案读取的产物证据，必须跟执行记录一起预览清理。' },
      { key: 'artBriefOutputs', label: '美术摘要 outputs/art-briefs', path: path.join(paths.root, 'outputs', 'art-briefs'), bytes: artBriefOutputsSize, protected: false, cleanupLevel: 'range', cleanupLevelLabel: '按范围可删', maintenanceType: 'art-briefs', actionLabel: '定位摘要清理', note: '任务中心生成的 HTML 摘要和 AI 工作说明，不影响禅道任务，但需按任务号、关键词或时间范围清理。' },
      { key: 'outputs', label: '输出目录 outputs', path: path.join(paths.root, 'outputs'), bytes: outputsSize, protected: true, cleanupLevel: 'caution', cleanupLevelLabel: '需慎重', maintenanceType: 'art-briefs', actionLabel: '定位摘要清理', note: '只允许维护中心按业务类型清理明确目录，不得手动删除整个 outputs。' },
      { key: 'logs', label: '运行日志 logs', path: path.join(paths.root, 'logs'), bytes: logsSize, protected: false, cleanupLevel: 'range', cleanupLevelLabel: '排障后可清', route: '/operation-logs', actionLabel: '查看操作日志', note: '排障窗口结束后可归档或截断；操作日志请走范围删除。' }
    ],
    records: [
      { key: 'operationLogs', label: '操作日志', count: Array.isArray(operationLogs) ? operationLogs.length : 0, file: paths.operationLogs, cleanupLevel: 'range', cleanupLevelLabel: '按范围可删', maintenanceType: 'operation-logs', route: '/operation-logs', actionLabel: '查看日志', note: '只删除审计日志，不回退 AI 产物累计调用次数。' },
      { key: 'runs', label: '执行记录 / AI档案', count: Array.isArray(runs) ? runs.length : 0, file: paths.runs, cleanupLevel: 'caution', cleanupLevelLabel: '需慎重', maintenanceType: 'runs', route: '/ai-archive', actionLabel: '查看 AI档案', note: '删除会移除非运行中的执行档案、工作区和产物归档，不删除任务中心任务。' },
      { key: 'tasks', label: '任务中心记录', count: Array.isArray(tasks) ? tasks.length : 0, file: paths.tasks, protected: true, cleanupLevel: 'protected', cleanupLevelLabel: '不要在维护中心删', route: '/tasks', actionLabel: '查看任务中心', note: '任务中心任务属于业务事实数据，不走维护中心范围清理。' },
      { key: 'artBriefs', label: '美术摘要索引', count: Array.isArray(artBriefs) ? artBriefs.length : 0, file: paths.artBriefs, cleanupLevel: 'range', cleanupLevelLabel: '按范围可删', maintenanceType: 'art-briefs', actionLabel: '定位摘要清理', note: '必须和 outputs/art-briefs 命中目录同步清理，不能单独手动删索引。' },
      { key: 'artProgressEvents', label: '研究同步记录', count: Array.isArray(artProgressEvents) ? artProgressEvents.length : 0, file: paths.artProgressEvents, protected: true, cleanupLevel: 'caution', cleanupLevelLabel: '需慎重', route: '/skills/events', actionLabel: '查看研究同步', note: '关联 AI 产物研究沉淀和同步记录，当前不纳入维护中心硬删除。' },
      { key: 'usageCounters', label: '累计调用指标桶', count: usageCounters && typeof usageCounters === 'object' ? Object.keys(usageCounters.buckets || usageCounters).length : 0, file: paths.usageCounters, protected: true, cleanupLevel: 'protected', cleanupLevelLabel: '禁止清理', actionLabel: '仅查看体量', note: '累计调用次数事实来源，删除其它明细也不得回退、清零或重算。' }
    ],
