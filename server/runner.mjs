...
function workflowInstruction(run) {
  ...
  return [
    ...
    `必须先尝试读取目标项目中的 .agent-hub/skills/${run.stage || '<技能名>'}/SKILL.md；如果该文件不存在，再按 .agent-hub/config.md、规范 md 线索和 AI 产物清单定位对应内容。`,
...
async function handleError(runId, error) {
  ...
  await updateRun(runId, {
    ...
    blocker: {
      ...
      impact: 'Codex runner failed before completing the workflow.',
      nextStep: 'Check codex CLI availability and rerun this task.'
    }
  });
