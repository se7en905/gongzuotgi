...
- 关键文件职责：
  ...
  - `src/views/AgentWorkersView.vue`：本机执行状态页，展示 Worker 在线、自检、准备就绪和执行明细。
...
- 直接执行到 Figma 界面规则：
  ...
  - `/agent-workers` 页面必须区分工作台管理者和组员美术执行者：
    ...
    - 非管理者登录时不得展示 `本机 Worker 绑定与实验操作` 整个模块。
    - 组员美术执行者不需要看到管理流程说明、命令分发说明和实验操作，只需要看到全员 Worker 心跳、准备就绪列表和执行明细。
  ...
  - `Worker 心跳` 模块必须按可见组员展示 Worker 信息；没有心跳的可执行组员也要展示，不得只循环已在线 Worker。
